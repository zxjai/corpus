...
def training_statistics(
    num_tokens=10**9, num_nonembed_params=10**9, gpu_type="h100", num_gpus=1, mfu=0.4
):
    ...
    table.add_row("Model size", f"{model_size}B params")
    table.add_row("Tokens", f"{num_tokens:.2e} tokens")
    table.add_row("GPU Type", f"{gpu_type}")
    table.add_row("Num GPUs", f"{num_gpus}")
    table.add_row("MFU", f"{mfu}")
    table.add_row("Days", f"{days:.2e}")
    table.add_row("Hours", f"{hours:.2e}")
    table.add_row("Minutes", f"{minutes:.2e}")
    table.add_row("Seconds", f"{seconds:.2e}")
    table.add_row("Cost", f"${dollars}")
...
def get_token_estimate(gb_size, lang="en", tokenizer_compression_ratio=4, log=True):
    ...
    if log:
        logger.info(
            f"{gb_size} GB {lanugage} dataset is approximately {num_tokens:.2e} tokens"
        )
    return num_tokens
